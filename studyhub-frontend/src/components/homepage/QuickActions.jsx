...
const QuickActions = ({ onFindTutor }) => {
    ...
    const handleFind = () => {
        onFindTutor?.({ rating: ratingFilters, price: priceFilters });
    };

    const activeCount = ratingFilters.length + priceFilters.length;
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${showFilter ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="max-w-2xl mx-auto bg-white/15 backdrop-blur-sm rounded-2xl px-6 py-5 text-left space-y-5">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {RATING_OPTIONS.map(opt => {
                                const active = ratingFilters.includes(opt.value);
                                return (
                                    <button key={opt.value}
                                        onClick={() => toggle(ratingFilters, setRatingFilters, opt.value)}
                                        className={`px-4 py-1.5 rounded-full text-sm font-semibold border-2 transition-all ${
                                            active ? 'bg-white text-blue-700 border-white'
                    ...
                    <div className="flex items-center justify-between pt-1">
                        {activeCount > 0 && (
                            <button onClick={() => { setRatingFilters([]); setPriceFilters([]); }}
